...
public class UserController {
    ...
    // Add validation for password fields to check strength of the password entered by the user at the time of registration. 
    ...
    public Boolean validatePassword(String password){
        if (password.matches("((?=.*\\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%]).{3,20})"))
            return true;
        return false;
    }
